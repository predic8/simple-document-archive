  ...
  withCredentials(
   ...
   withCredentials(
    [
      ...
      sh "docker run --name $containerName -d -v /var/run/docker.sock:/var/run/docker.sock -e HUB_USER=$HUB_USER -e HUB_PASSWORD=$HUB_PASSWORD -e APPLICATION_PROPERTIES="echo ${APPLICATION_PROPERTIES}") $tempImageName sleep 1000000"
   }
}
